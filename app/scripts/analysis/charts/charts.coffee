...
.controller('mainChartsCtrl', [
  ...
  (ctrlMngr,$scope) ->
    _chart_data = null
    _chart_data_reformat = null
    _headers = null
    _name = null

    _updateData = () ->
      #toggle between two local variables for different data formats
      $scope.chartData = _chart_data_reformat
    ...
    $scope.$on 'charts: send reformat', (event, data) ->
      _chart_data_reformat = data
      _updateData()

    $scope.$on 'charts: reformat headers', (event, data) ->
      _headers = data
      _updateData()

    $scope.$on 'charts: reformat name', (event, data) ->
      _name = data
...
.controller('sideChartsCtrl',[
  ...
  (ctrlMngr, $scope, $rootScope, $stateParams, $q, dataTransform, list, sendData,time) ->
    ...
    $scope.changeName = () ->
      ...
      $rootScope.$broadcast 'charts: send reformat', reformat
      $rootScope.$broadcast 'charts: reformat name', $scope.graphInfo.graph
...
#        sendData.createGraph(_chartData, $scope.graphInfo, _headers, $rootScope, $scope.dataType, $scope.selector4.scheme)
    ...
    $scope.changeVar = (selector,headers, ind) ->
      selected = [$scope.selector1, $scope.selector2, $scope.selector3]
      $rootScope.$broadcast 'charts: reformat headers', selected
      ...
      listener: (msg, _data) ->
        switch _data.dataType
          when "FLAT"
            ...
            reformat = dataTransform.format2(_chartData, _data.header)
#            $rootScope.$broadcast 'charts: send reformat', reformat
...
.factory('app_analysis_charts_dataTransform',[
  () ->
    ...
    _format2 = (data, header) ->
      tmp = []
      for d,i in data
        obj = {header: header[i], data: d}
        tmp.push obj
      tmp
...
.directive 'd3Charts', [
  ...
  'stackBar',
  'app_analysis_charts_checkTime'
  (scatterPlot, histogram, pie, bubble, bar, streamGraph, area, treemap, line, bivariate, stackBar, time) ->
    restrict: 'E'
    ...
    link: (scope, elem, attr) ->
      ...
      scope.$watch 'headers', (newHeaders) ->
        if newHeaders
          headers = newHeaders
          varNames = headers.map (h) -> h.value
          check  = (col) ->
            return col.header in varNames
          console.log gdata
          dataReformat = gdata.filter (check)
...
#          rangesReformat =
#            xMin: d3.min gdata.data, (d) -> parseFloat d
#            yMin: d3.min gdata.data, (d) -> parseFloat d
#
#            xMax: d3.max gdata.data, (d) -> parseFloat d
#            yMax: d3.max gdata.data, (d) -> parseFloat d
        ...
        if newName
          ...
          switch name
            ...
            when 'Stacked Bar Chart'
              alert "called"
              stackBar.stackedBar(dataReformat,width,height,_graph, gdata,container)
            when 'Stream Graph'
